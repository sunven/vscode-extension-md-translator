...
function activate(context) {
    ...
    // 注册命令
    const convertFileCommand = vscode.commands.registerCommand('jsToJsx.convertFile', async (uri) => {
        try {
            await convertJsToJsx(uri);
        }
        catch (error) {
            vscode.window.showErrorMessage(`Failed to convert file: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    });
    context.subscriptions.push(convertFileCommand);
}
exports.activate = activate;
async function convertJsToJsx(uri) {
    ...
    if (fs.existsSync(newFilePath)) {
        const choice = await vscode.window.showWarningMessage(`File ${fileName}.jsx already exists. Do you want to replace it?`, 'Yes', 'No');
        if (choice !== 'Yes') {
            return;
        }
    }
