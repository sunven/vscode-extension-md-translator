...
function activate(context) {
    ...
    const translateMarkdownCommand = vscode.commands.registerCommand('fileExtensionConverter.translateMarkdownToChinese', async (uri) => runWithErrorBoundary('Failed to translate Markdown', () => (0, translateMarkdown_1.translateMarkdownToChinese)(context, translatedMarkdownProvider, uri)));
    const setApiKeyCommand = vscode.commands.registerCommand('fileExtensionConverter.setApiKey', async () => (0, config_1.promptAndStoreApiKey)(context));
    const clearApiKeyCommand = vscode.commands.registerCommand('fileExtensionConverter.clearApiKey', async () => (0, config_1.clearApiKey)(context));
