...
exports.decodeAssistantContent = exports.parseTranslationResponse = exports.applyDisableThinkingHint = exports.translateSegmentsWithOpenAI = void 0;
...
async function translateSegmentsWithOpenAI(options, segments) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new translationShared_1.TranslationClientError(buildProviderErrorMessage(response.status, responseText), response.status);
        }
        ...
        if (error instanceof Error && error.name === 'AbortError') {
            throw new translationShared_1.TranslationClientError('Translation request timed out.');
        }
        throw new translationShared_1.TranslationClientError(error instanceof Error ? error.message : 'Unknown provider error.');
    }
    finally {
...
function parseTranslationResponse(content, expectedIds) {
    ...
    try {
        ...
        throw new translationShared_1.TranslationClientError(`Provider returned invalid JSON: ${error instanceof Error ? error.message : 'Unknown parse error'}`);
    }
    if (!isTranslationResponse(parsed)) {
        throw new translationShared_1.TranslationClientError('Provider response must contain a translations array.');
    }
    const expected = new Set(expectedIds);
    const translations = new Map();
    for (const item of parsed.translations) {
        if (!expected.has(item.id)) {
            throw new translationShared_1.TranslationClientError(`Provider returned unknown segment id: ${item.id}`);
        }
        if (translations.has(item.id)) {
            const existing = translations.get(item.id);
            const normalized = (0, translationShared_1.normalizeTranslatedText)(item.text);
            if (existing === normalized) {
                continue;
            }
            throw new translationShared_1.TranslationClientError(`Provider returned conflicting translation for segment id: ${item.id}`);
        }
        translations.set(item.id, (0, translationShared_1.normalizeTranslatedText)(item.text));
    }
    for (const id of expected) {
        if (!translations.has(id)) {
            throw new translationShared_1.TranslationClientError(`Provider response is missing segment id: ${id}`);
        }
    }
    return translations;
}
...
function decodeAssistantContent(responseText, contentType) {
    ...
    try {
        ...
        throw new translationShared_1.TranslationClientError(`Provider returned invalid completion JSON: ${error instanceof Error ? error.message : 'Unknown parse error'}`);
    }
    const content = getPath(parsed, ['choices', 0, 'message', 'content']);
    if (typeof content !== 'string' || !content.trim()) {
        throw new translationShared_1.TranslationClientError('Provider response did not include assistant message content.');
    }
    return content;
}
...
function extractAssistantContentFromEventStream(responseText) {
    ...
    for (const line of responseText.split(/\r?\n/)) {
        ...
        try {
            ...
            throw new translationShared_1.TranslationClientError(`Provider returned invalid streaming JSON: ${error instanceof Error ? error.message : 'Unknown parse error'}`);
        }
    ...
    if (pieces.length === 0) {
        throw new translationShared_1.TranslationClientError('Provider returned a streaming response without assistant content.');
    }
    return pieces.join('');
}
function extractJsonObjectText(content) {
    ...
    if (firstBrace === -1 || lastBrace === -1 || lastBrace < firstBrace) {
        throw new translationShared_1.TranslationClientError('Provider response did not contain a JSON object.');
    }
